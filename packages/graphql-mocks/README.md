...
// scenarios.default | scenarios.loading | scenarios.errored — each MockHandlerOptions
```
...
const scenarios = mockScenarios({}, 'UserById');

export const Default = { parameters: { graphqlMocks: scenarios.default } };
export const Loading = { parameters: { graphqlMocks: scenarios.loading } };
export const Errored = { parameters: { graphqlMocks: scenarios.errored } };
```
...
| `listSize` | `number \| { min: number, max: number }` | `{ min: 1, max: 5 }` | How many items generated list fields hold |
